...
#add in rurality status to data_qld file 

#remove un-needed columns from rural ranking data_qld set 
dplyr::select(iris, Sepal.Width, Petal.Length, Species)


#load rurality file 
rural <- read.csv("/Volumes/robertj9/L.RProjects/L.Uncert.spatialmap/SLA_2006asgc.csv")
...
## add rurality & socioeconomi to data_qld file 
data_qld$ariap <- rural$ariap_cat
data_qld$IRSD <- rural$Seifa06_IRSD
data_qld$IRSAD <- rural$Seifa06_IRSAD

#Create variable for CI half Length 
data_qld <- dplyr::mutate(data_qld, ci.half = ci.length/2) 
...
## using a ratio of the estimate over the variance we can attempt to adjust for the increased variance as the estimate increases.  scale varialbe is a ration of est/variance 

x.var <- (data_qld$ci.length/4)^.5
plot(data_qld$est, x.var)
data_qld$x.var <- x.var

##create scale variable using x.var 
test.scale <- data_qld$est/x.var
plot()
plot(data_qld$est, test.scale)
###add test.scale to data_qld database 
data_qld$test.scale <- test.scale
...
##action1 = 
data_qld <- data_qld %>% mutate(action = ifelse(est >=(ci.length/2 +1) & ci.length < 2,  1, NA))
data_qld <- data_qld %>% mutate(action = ifelse(est <(ci.length/2+1) & (est>(0.5-ci.length)*2), 2, data_qld$action))
data_qld <- data_qld %>% mutate(action = ifelse(est<=(-0.5*ci.length + 1), 3, data_qld$action))
data_qld <- data_qld %>% mutate(action = ifelse(ci.length >= 2, 4, data_qld$action))

##action2  = 
data_qld <- data_qld %>% mutate(action2 = ifelse(est >=(ci.half +1),  1, NA))
data_qld <- data_qld %>% mutate(action2 = ifelse(est <(ci.half+1) & (est>(0.5-ci.length)*2), 3, data_qld$action2))
data_qld <- data_qld %>% mutate(action2 = ifelse(est <(ci.half+1) & (est>(0.5-ci.length)*2) & ci.l >= 0.85, 2, data_qld$action2))
data_qld <- data_qld %>% mutate(action2 = ifelse(est<=(-0.5*ci.length + 1), 4, data_qld$action2))
data_qld <- data_qld %>% mutate(action2 = ifelse(ci.half >= .8, 5, data_qld$action2))

##action3 = uses test.scale to scale the credible interval length as est increases. 
data_qld <- data_qld %>% mutate(action3 = ifelse(est >= 1 & (test.scale <=2.5), 1, NA))
data_qld <- data_qld %>% mutate(action3 = ifelse(est >= 1 & (test.scale >2.5), 2, data_qld$action3))
data_qld <- data_qld %>% mutate(action3 = ifelse(est <1 & (test.scale <= 2.5), 3, data_qld$action3))
...
######
# data_qld <- data_qld %>% mutate(action2 = ifelse(est >1.0 & ci.l >= 0.95, 2, NA))
# data_qld <- data_qld %>% mutate(action2 = ifelse(est >1.0 & ci.l >= 1.05, 1, data_qld$action2 ))
# data_qld <- data_qld %>% mutate(action2 = ifelse(est <(ci.half+1) & (est>(0.5-ci.length)*2) & ci.l >= 0.85, 3, data_qld$action2))
# data_qld <- data_qld %>% mutate(action2 = ifelse(est <1.0 & ci.u >= 0.95, 4, data_qld$action2))
# data_qld <- data_qld %>% mutate(action2 = ifelse(est < 1.0 & ci.u < 1.00, 5, data_qld$action2))
# data_qld <- data_qld %>% mutate(action2 = ifelse(est< 1.0 & ci.u > 1.15, 2, data_qld$action2))
# data_qld <- data_qld %>% mutate(action2 = ifelse(ci.half >= 1, 6, data_qld$action2))
# 


#write data_qld to csv file in L.uncer.spatial folder.
write.csv(data_qld, '/Volumes/robertj9/L.RProjects/L.Uncert.spatialmap/BDVAmap.SLA.sept2016.csv', row.names=F) 


#scatterplot f ci.length vs risk estimate

p.est.ci <- ggplot(data_qld, aes(est, ci.half))
...
p2.est.ci <- ggplot(data_qld, aes(ci.half, est))
p2.est.ci + geom_point()
p2.est.ci + geom_linerange(limits1)
...
##ggplot version 
plot.var <- ggplot(data_qld, aes(est, x.var))
plot.var<- plot.var + geom_point()
plot.var


#showing lower bound vs ci.length. 
lower <- ggplot(data_qld, aes(ci.length, ci.l)) + geom_point()
lower
...
upper <- ggplot(data_qld, aes(ci.length, ci.u)) + geom_point()
...
#facet_grid 
mt <- ggplot(data_qld, aes(ci.length, p.est.ci, colour = factor(ariap))) + geom_point()
